...
class TransactionConfirmationTest {
    ...
    @Test fun confirmationIsDurableOnceOnlyAndLinksToCommittedTransaction() = runBlocking {
        ...
        assertNotNull(first)
        assertEquals("Lumi · Pembayaran tercatat", first.extras.getCharSequence(Notification.EXTRA_TITLE))
        assertNotNull(first.getLargeIcon())
        assertNotNull(first.publicVersion.getLargeIcon())
    ...
    @Test fun hiddenAmountsAndTransferUseOnePrivateConfirmation() = runBlocking {
        ...
        assertEquals("Lumi · Transfer tercatat", posted.extras.getCharSequence(Notification.EXTRA_TITLE))
