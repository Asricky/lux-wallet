...
class CoachNotificationTest {
    @Test fun notificationRequiresPermissionAndHidesFinancialDetailOnLockScreen() {
        ...
        assertNotNull(posted.getLargeIcon())
        assertNotNull(posted.publicVersion.getLargeIcon())
        assertEquals(com.luxwallet.app.R.drawable.ic_notification_lumi, posted.smallIcon.resId)
        assertTrue(posted.extras.getCharSequence(Notification.EXTRA_TITLE).toString().startsWith("Lumi"))
