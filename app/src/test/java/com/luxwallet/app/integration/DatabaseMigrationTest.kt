...
class DatabaseMigrationTest {
    @Test fun versionOneDataSurvivesVersionTwoMigration() = runBlocking {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val name = "migration-test.db"
        context.deleteDatabase(name)
        val stream = javaClass.classLoader!!.getResourceAsStream("com.luxwallet.app.core.database.LuxDatabase/1.json")!!
        val schema = stream.bufferedReader().use { Json.parseToJsonElement(it.readText()).jsonObject["database"]!!.jsonObject }
        val config = SupportSQLiteOpenHelper.Configuration.builder(context).name(name).callback(object : SupportSQLiteOpenHelper.Callback(1) {
        ...
        val db = Room.databaseBuilder(context, LuxDatabase::class.java, name).addMigrations(LuxDatabase.MIGRATION_1_2).allowMainThreadQueries().build()
        try {
